var myTitle = "IsoCubes On Chain"
console.log(myTitle + " | smldms 2023.11")
...
function rdmColor(palette, a) {
    ...
    const couleurHex = couleurs[Math.floor(fxrand() * couleurs.length)];
    const alpha = a;
...
function drawCube(isoX, isoY) {
    ...
    ctx.strokeStyle = rdmColor(plt.clr, fxrand() * 255);
...
function drawScene() {
    ...
    for (let x = -cnv.width / 2; x < cnv.width / 2; x += scl) {
        for (let y = -cnv.height / 2; y < cnv.height / 2; y += scl) {
            ...
            IsoH = IsoW * 0.5;
            drawCubes(x, y, Math.floor(fxrand() * area.val), fxrand() * 1.618);
        }
    }
